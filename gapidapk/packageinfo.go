...
func PackageList(ctx context.Context, d adb.Device, includeIcons bool, iconDensityScale float32) (*pkginfo.PackageList, error) {
	...
	log.W(ctx, "Looking for service action...")
	...
	log.I(ctx, "Starting service... only debug is %s", onlyDebug)
	...
	log.W(ctx, "Unmarshalling data...")

	out := &pkginfo.PackageList{}
	log.W(ctx, "befor Unmarshal out value is %s", out)

	if err := jsonpb.Unmarshal(sock, out); err != nil {
		return nil, log.Err(ctx, err, "unmarshal json data")
	}
	//	log.W(ctx, "after Unmarshal out is :%s", out)
	log.W(ctx, "Sorting data...")
	out.Sort()
	//out = nil
	//log.W(ctx, "out value is %s", out)
