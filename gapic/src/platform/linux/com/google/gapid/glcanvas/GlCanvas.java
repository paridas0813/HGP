...
//import static com.google.gapid.util.Paths.toString;
...
public abstract class GlCanvas extends Canvas {
  ...
  private boolean createContext() {
    ...
    LOG.log(INFO, "begine createCapabilitiesGLX !!!");
    //GLXCapabilities glxCaps = GL.createCapabilitiesGLX(display, screen);

    //test
    GLXCapabilities glxCaps;
  
    LOG.log(SEVERE, "display is " + display + "screen: " + screen);
    ...
    long config = chooseConfig(display);
    if (config == -1) {
      return false;
    }
  ...
  private static long chooseConfig(long display) {
