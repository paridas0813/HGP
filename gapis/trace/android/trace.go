...
func (t *androidTracer) ProcessProfilingData(ctx context.Context, buffer *bytes.Buffer,
	...
	log.W(ctx, " !!! begine  ProcessProfilingData !!!!!.")
	...
	if strings.Contains(gpuName, "Adreno") {
		...
		log.W(ctx, " !!!Adreno ProcessProfilingData !!!!!.")
...
func (t *androidTracer) Validate(ctx context.Context, enableLocalFiles bool) (*service.DeviceValidationResult, error) {
	...
	log.W(ctx, "Validating device (%v) with (%v) validator", osConfiguration.GetHardware().GetGPU().GetName(), t.v.GetType())

	// Get ActivityAction
	gapidPackage := gapidapk.PackageName(osConfiguration.PreferredABI(nil))
	log.W(ctx, "gapidpackage name is %s activityName name is %s befor invok InstalledPackages", gapidPackage, activityName)
	...
	status.Do(ctx, "Trace loading", func(ctx context.Context) {
		...
		log.W(ctx, "Writing trace size %v bytes to %v", numWritten, file.Name())
