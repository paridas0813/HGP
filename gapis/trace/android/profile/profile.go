...
func (pd *ProfilingData) ComputeCounters(ctx context.Context) {
	...
	log.W(ctx, "pd.slices size is %d", len(pd.Slices))
	...
	log.W(ctx, "befor  setGupCounterMetrics  current groupToSlices size is %d, pd.counters size is %d  filteredSlices is %d", len(groupToSlices), len(pd.Counters), len(filteredSlices))
...
func setGpuCounterMetrics(ctx context.Context, groupToSlices map[int32][]*Slice,
	...
	log.W(ctx, "groupslice is %d , src metrics size is %d!!!!!!", len(groupToSlices), len(*metrics))

	for i, metric := range *metrics {

		log.E(ctx, "metricId %d ,counter.Name is : %s, metricId : %d, Unit: %s", i, metric.Name, metric.Id, metric.Unit)
	}
	...
	for i, counter := range counters {
		...
		log.E(ctx, "counter.Name is : %s, metricId : %d, counter.Id:%d, Unit: %s, description: %s", counter.Name, metricId, counter.Id, counter.Unit, description)
		...
		*metrics = append(*metrics, counterMetric)
		...
		log.W(ctx, "concurrentSlicesCount  size is %d  ", len(concurrentSlicesCount))

		counterPerfSum, counterPerfAvg := float64(0), float64(-1)
		//遍历全局切片，将计数器样本映射到 GPU 切片。在收集样本时，基于归因策略，会维护三个集合：最小集、最佳猜测集和最大集。函数返回的结果是三个映射，将样本索引映射到样本权重。
		for groupId, slices := range groupToSlices {
		...
		log.E(ctx, "counterMetric.Average is : %f", counterMetric.Average)
...
func (pd *ProfilingData) MergeStaticAnalysis(ctx context.Context, staticAnalysis *api.StaticAnalysisProfileData) {
	...
	log.W(ctx, "counterData size  is : %d", len(staticAnalysis.CounterData))

	for _, samples := range staticAnalysis.CounterData {

		group := pd.Groups.GetOrCreateDrawCallGroup("draw call", samples.Index)

		log.W(ctx, "samples.id is : %d , group is %d", samples.Index, group)
		if group < 0 {
